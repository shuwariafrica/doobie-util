 ...
 * License.                                                     *
 ****************************************************************/
package ashtray.mssql.test.time
...
class test_formatters extends munit.FunSuite:
  ...
  test("'formatter.datetime2' rejects over-precision fractional seconds") {
    intercept[Exception] {
      LocalDateTime.parse("2024-02-23 23:33:01.786401300", formatter.dateTime2)
    }
  }

  test("'formatter.datetimeoffset' rejects malformed offsets") {
    intercept[Exception] {
      OffsetDateTime.parse(datetime2String + " +0300", formatter.datetimeoffset)
    }
  }
end test_formatters
